...
shuffleBtn.addEventListener('click', ()=>{ shuffleSolvable(); gameActive = true; });
resetBtn.addEventListener('click', ()=>{ reset(); gameActive = true; });
...
solveBtn.addEventListener('click', async ()=>{
  ...
  if(!result || !result.path){ statusEl.textContent = 'No solution (unsolvable)'; solutionPath=null; document.getElementById('solver-stats').textContent = `Nodes explored: ${result ? result.nodesExplored : 0}; Time: ${result ? result.timeMs.toFixed(1) : 0} ms`; return; }
  ...
  const anim = setInterval(()=>{
    if(i>=result.path.length){ clearInterval(anim); return; }
...
saveBtn.addEventListener('click', ()=>{
  const key = 'pathfinder8-latest';
  localStorage.setItem(key, JSON.stringify({n, state, moves}));
  statusEl.textContent = 'Saved to localStorage';
});
loadBtn.addEventListener('click', ()=>{
  const key = 'pathfinder8-latest';
  const raw = localStorage.getItem(key);
  if(!raw){ statusEl.textContent='No saved configuration'; return; }
  try{ const obj=JSON.parse(raw); n=obj.n; sizeEl.value=n; state=obj.state; moves=obj.moves||0; layoutBoardContainer(); createTiles(); updateTiles(); statusEl.textContent='Loaded from localStorage'; }catch(e){ statusEl.textContent='Load failed'; }
});
exportBtn.addEventListener('click', ()=>{
  const payload = { n, state, moves, timestamp: new Date().toISOString() };
  const blob = new Blob([JSON.stringify(payload, null, 2)], {type:'application/json'});
  const url = URL.createObjectURL(blob);
  const a = document.createElement('a'); a.href=url; a.download='pathfinder8-puzzle.json'; a.click(); URL.revokeObjectURL(url);
});
importBtn.addEventListener('click', ()=>{ importFile.click(); });
importFile.addEventListener('change', (e)=>{
  const f = e.target.files && e.target.files[0]; if(!f) return;
  const reader = new FileReader(); reader.onload = ()=>{
    try{ const obj = JSON.parse(reader.result); if(obj.n && obj.state){ n=obj.n; sizeEl.value=n; state=obj.state; moves=obj.moves||0; layoutBoardContainer(); createTiles(); updateTiles(); statusEl.textContent='Imported file'; } else statusEl.textContent='Invalid file'; }catch(err){ statusEl.textContent='Import failed'; }
  }; reader.readAsText(f);
});
